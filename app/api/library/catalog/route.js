    ...
    await initDb();
		...
		if (userId) {
			const rows = await query(
				`SELECT ws.id, ws.title, ws.audio_filename, ws.video_filename, ws.thumbnail_filename, ws.language, ws.created_at,
			...
			const rows = await query(
				`SELECT id, title, audio_filename, video_filename, thumbnail_filename, language, created_at, 0 AS inLibrary, '' AS tags
